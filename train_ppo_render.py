...
def train(
    ...
    print('  - higher entropy (0.1) for more exploration')
    print('  - higher learning rate (1e-3) for faster learning')
    print('  - smaller batches (32) for stability')
    print('  - attack bonus reward for encouraging offense')
    ...
    log_file = f'logs/training_mk2_improved_{timestamp}.txt'
    ...
    for episode in range(num_episodes):
        ...
        while not done and episode_steps < max_steps_per_episode:
            ...
            # add small bonus for attacking to encourage offense
            if attack_action > 0:
                reward += 0.5
        ...
        if episode % save_interval == 0 and episode > 0:
            save_path = f'saved_models/ppo_mk2_improved_episode_{episode}.pth'
            agent.save(save_path)
            print(f'model saved to {save_path}')
    
    final_save_path = f'saved_models/ppo_mk2_improved_final_{timestamp}.pth'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--episodes', type=int, default=2500, help='number of episodes')
