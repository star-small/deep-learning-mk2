...
def train(
    ...
    print('  - moderate entropy (0.03) for balance')
    print('  - standard learning rate (3e-4)')
    print('  - infrequent updates (2048 steps) for speed')
    ...
    log_file = f'logs/training_mk2_cpu_{timestamp}.txt'
    ...
    for episode in range(num_episodes):
        ...
        while not done and episode_steps < max_steps_per_episode:
            ...
            # diversity bonus - reward for using different attacks
            if len(last_actions) > 0 and attack_action != last_actions[-1]:
                reward += 0.1
            
            last_actions.append(attack_action)
            ...
            if total_steps % update_interval == 0 and len(agent.rollout_buffer['states']) > 0:
                print(f'  updating at step {total_steps}...')
        ...
        if episode % save_interval == 0 and episode > 0:
            save_path = f'saved_models/ppo_mk2_cpu_episode_{episode}.pth'
            agent.save(save_path)
            print(f'model saved to {save_path}')
    
    final_save_path = f'saved_models/ppo_mk2_cpu_final_{timestamp}.pth'
